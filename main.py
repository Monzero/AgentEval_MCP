...
class OptimizedConfig:
    ...
    def __init__(self, company: str, base_path: str = None):
        ...
        # Add these lines to OptimizedConfig.__init__() method in main.py
        # Around line 180-200, after the existing agent_temperatures setup

        # Dynamic Orchestration Configuration
        self.enable_dynamic_orchestration = True  # Enable by default for Option A

        # Specialized LLMs for dynamic orchestration
        self.agent_llms.update({
            "planner": "gemini-1.5-pro",      # Strategic workflow planning (needs high intelligence)
            "orchestrator": "gemini-1.5-flash"  # Real-time execution decisions (needs speed)
        })

        self.agent_temperatures.update({
            "planner": 0.2,        # Low temperature for consistent planning
            "orchestrator": 0.1    # Very low for consistent execution decisions
        })

        # Dynamic orchestration settings
        self.dynamic_orchestration = {
            "max_parallel_steps": 5,           # Max steps to run in parallel
            "replanning_threshold": 0.5,       # When to trigger re-planning (50% failure rate)
            "performance_history_size": 10,    # Executions to track per agent
            "capability_discovery_cache": 300, # Seconds to cache capabilities
            "enable_workflow_adaptation": True, # Allow workflow re-planning
            "parallel_execution_enabled": True # Enable parallel step execution
        }

        # Workflow preferences
        self.workflow_optimization = {
            "prefer_parallel": True,           # Prefer parallel execution when possible
            "skip_unnecessary_validation": False, # Always validate for safety
            "adaptive_iteration_count": True,  # Let AI decide iteration count
            "fallback_to_static": True        # Fallback to static workflow if AI planning fails
        }
